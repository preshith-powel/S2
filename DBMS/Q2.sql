...
-- SQL QUERIES TO DISPLAY THE COUNTRIES
...
-- WRITE QUERY TO FIND THE HIRE DATE FOR EMPLOYEES WHOSE LAST NAME IS "GRANT" OR "WHALEN".
select 
concat(employees.first_name," ",employees.last_name) as name,
hire_date
from employees
where last_name in ("whalen","grant");
...
-- 7) write a query to display the department from descending order
...
select department_name,department_id from dept
order by department_id asc;

-- 8) query to display all the employees whose lastname starts with "k"
...
-- 9) display the names of the employees whose hire dates are between 1995 and 1997

select
concat(employees.first_name ," ",employees.last_name),hire_date as name
from employees 
...
concat(employees.first_name ," ",employees.last_name),hire_date as name
from employees 
where weekday(hire_date)=2;

-- max salary less than 5000;
...
-- --------------------------------------------------------------------------
select * from dept where department_name="shipping";
delete from employees where manager_id=100;
